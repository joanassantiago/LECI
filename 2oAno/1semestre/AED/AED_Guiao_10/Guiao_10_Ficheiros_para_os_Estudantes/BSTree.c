...
// 4) Depth-first post-order?    Esta é que está correta 
// A: ...
// Q: Is this the best order here? Why?   Sim, porque temos assim garantia que os ponteiros são válidos, pois se tirarmos a root nao temos garantia
// A: ...
...
static int _treeGetHeightREC(const struct _BSTreeNode* root) {
  // COMPLETE
  // ...
  //árvore vazia devolve-se -1
  //raiz é uma folha então devolve-se 0
  //raiz só tem filhos esq
  //raiz só tem filhos dir
  //caso geral

  if(root == NULL){
    return -1;
  }
  int height_left = _treeGetHeightREC(root->left);
  int height_right = _treeGetHeightREC(root->right);
  return 1 + ((height_left > height_right) ? height_left : height_right); //mais compacto
...
void* BSTreeGetMin(const BSTree* header) {
  ...
  // ...
  // Nó de menor valor é o " nó mais á esquerda"
...
static void* _treeGetMax(const struct _BSTreeNode* root) {
  ...
  // ...
  // Nó de meior valor é o " nó mais á direita"
  if(root->right == NULL){ // se nao existir mais nenhum numero a direita devolve-se esse item 
    return root->item;
  }
...
static void _BSTreeAddItems(const struct _BSTreeNode* p, Queue* q) {
  ...
  // COMPLETE
  // ...
  _BSTreeAddItems(p->left,q); //toda a sub-arvore esquerda
  QueueEnqueue(q,p->item);
  _BSTreeAddItems(p->right,q);  //toda a sub-arvore ditreita
}
// Q: What kind of tree traversal is this function doing?
// 1) Breadth-first,
// 2) Depth-first pre-order,

...
GraphDFSWithStack* GraphDFSWithStackExecute(Graph* g,
  ...
  Stack * stack = StackCreate(GraphGetNumVertices(g));
  traversal->marked[startVertex] = 1;
  StackPush(startVertex);
  while(StackIsEmpty(stack) == 0){
    int vertex = StackPop(stack);
    unsigned int * neighbors = GraphGetAdjacentsTo(graph, vertex);
    for(int i = 1; i <= neighbors[0]; i++){
        int neighbor = neighbors[i];
        if(traversal->marked[neighbor] == 0);
          traversal->marked[neighbor];
          StackPush(neighbor);
    }
  }
  free(neighbors);

...
void MinHeapInsert(MinHeap* ph, void* item) {
  ...
  while (n > 0) {
    ...
    if (ph->compare(ph->array[p], item) <= 0) break;
...
int MinHeapCheck(MinHeap* ph) {
  ...
  for (unsigned int n = 1; n < ph->size; n++) {
    int p = _parent(n);
    if (ph->compare(ph->array[p], ph->array[n]) < 0) return 0;
  }
  return 1;
}

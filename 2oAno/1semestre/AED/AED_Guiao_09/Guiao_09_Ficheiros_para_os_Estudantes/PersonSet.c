...
PersonSet *PersonSetCreate() {
  ...
  // COMPLETE ...
  PersonSet * result = (PersonSet *) malloc (sizeof(PersonSet));
  result->persons = ListCreate(cmpP);
...
void PersonSetAdd(PersonSet *ps, Person *p) {
  // COMPLETE ...
  if(ListSearch(ps->persons,p )== 0) return;
...
Person *PersonSetRemove(PersonSet *ps, int id) {
  ...
  Person * result = NULL;
  person.id = id;
  if(ListSearch(ps->persons,&person) == 0){
    result = ListGetCurrentItem(ps->persons);
  }
  return NULL;
}
...
PersonSet *PersonSetUnion(const PersonSet *ps1, const PersonSet *ps2) {
  ...
  // COMPLETE ...
    ...
    for(int i = 0; i < ListGetSize(ps2->persons); i++){
      ListMove(ps2->persons, i);
